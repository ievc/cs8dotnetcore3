...
namespace LinqWithEFCore
{
    class Program
    {
        ...
        static void FilterAndSort()
        {
            using (var db = new Northwind())
            {
                var query = db.Products
                    .ProcessSequence()
                    .Where(product => product.UnitPrice < 10M)          // IQueriable<Product>
        ...
        static void JoinCategoriesAndProjects()
        {
            using (var db = new Northwind())
            {
                // join every product with its category
